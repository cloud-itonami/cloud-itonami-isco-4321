...
gap is now closed (`test/warehouse_stock/actor_test.cljk`).
...
- `src/warehouse_stock/store.cljk` — `Store` protocol + `MemStore` +
  ...
  (`test/warehouse_stock/store_contract_test.cljk`).
- `src/warehouse_stock/advisor.cljk` — `Advisor` protocol; `mock-advisor`
...
- `src/warehouse_stock/governor.cljk` — `WarehouseStockGovernor`:
...
- `src/warehouse_stock/actor.cljk` — `build-graph`, `run-request!`,
